...
//↓↓共有機能群↓↓
  ...
  this.LINE    = (text_str,token_str,way_str,to_str) => {
    /*  way_str    to_str
        reply      リプライトークン
        push       対象のID
        multicast  対象のID配列
        broadcast　-
    */
    Array.isArray(text_str)||(text_str = [text_str]);
    UrlFetchApp.fetch(`https://api.line.me/v2/bot/message/${way_str}`,{method :"post",headers:{"Content-Type" :`application/json`,"Authorization":`Bearer ${token_str}`},
        payload:JSON.stringify(Object.assign({"messages":text_str.map(el=>({"type":"text","text":el}))},
          way_str===`reply` ? {"replyToken":to_str} : way_str===`push`||way_str===`multicast` ? {"to":to_str} : {}
    ))});
  };
  this.DB      =                                        {
    color:{
  ...
  this.CONF    =                                        {
    ...
    site     :{
      ls : [
        ["http://sukasuka-anime.com"        ,"すかすか Home" ],
        ["http://sukasuka-anime.com/news/"  ,"すかすか News" ],
        ["http://sukasuka-anime.com/goods/" ,"すかすか Goods"],
        ["https://anime-86.com/news/"       ,"エイティシックス News"]
      ],
    },
    trainDly :{
      jr : {
        get : [1,1,1],//遅延証明書の掲載順　　取得:1　それ以外:0 or undefined
        ls  : ["東海道線","横須賀線","京浜東北線","横浜線","南武線"],
      },
    },
    schedule :{
      shortTt : {
...
//↓↓　処理部　↓↓
  ...
  this.exe_HTTP = (bootInfo,boottime) => {
    this.exe_HachimakiBot=(data)=>{
      ...
      if(JSON.parse(data.postData.contents).events){JSON.parse(data.postData.contents).events.forEach(event=>{if(event.type==`message`&&event.message.type===`text`){
        ...
        : RegExp(`^(おはよう|お早う|こんにちは|こんばんは|よろしく).*(はちまき|ハチマキ)(ぼっと|ボット|bot)`,`u`).test(txt) ? `HelloWorld !`
        //非適合の場合
        : txt === `FAIL(､´･ω･)▄︻┻┳═一💥 [cmd]彡` ? `撃っていいのは撃たれる覚悟のある奴だけだ\n(､´･ω･)▄︻┻┳═一💥💥💥💥💥`
        : txt === `撃っていいのは撃たれる覚悟のある奴だけだ\n(､´･ω･)▄︻┻┳═一💥💥💥💥💥` ? `　 ＿∧_∧＿＿＿/／\n≡(_ ( ･∀･)＿＿( 三X))≡＝●\n　　( ニつノ　　＼\n　　ヽ_⌒|\n　　し_(＿)`
        : event.source.groupId ? null : `FAIL(､´･ω･)▄︻┻┳═一💥 [cmd]彡`;
        message&&LINE(message,CONF.line.token.HachimakiBot,`reply`,event.replyToken);
    ...
    return this[`exe_${bootInfo.func[1]}`](bootInfo.data,bootInfo.time);
  };
//↑↑　処理部　↑↑
