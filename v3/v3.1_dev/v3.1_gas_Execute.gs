    ...
    メール転送: function()
      {
        GmailApp.getMessagesForThreads(GmailApp.search("is:unread",0,20)).forEach(スレッド=>{
          ...
          if(設定.メール転送.ブラリス.reduce((A,B)=>{A ? メッセージ[0][B[0]]().includes(B[1]) : A},true)){
            if(Parser.data(スレッド[0].getFrom()).from("<").to(">").build() || スレッド[0].getFrom() == "no-reply@classroom.google.com"){
    ...
    スケジュール: function(arg)
      {
        ...
        for(let i = 0; i < 7; i++){
          ...
          予定表[i]=予定表[i].slice(2)
          if(予定表[i][0]) 予定[i].詳細.push(["行事",`${予定表[i][0]}`])
       ...
       if(arg){
        関数.LINE.送信(
          [/* 予定.reduce((前回,要素)=>前回+=`\n${要素.日付}　${要素.天気}\n　${要素.概要.reduce((A,B)=>{return A+=`\n　${B}`})}`,"【1週間の予定】"), */
           予定[0].詳細.reduce((A,B)=>A+=`\n　${B[1] ? `${B[0]}　${B[1]}` : B[0]}`,`【${予定[0].日付}】`)],
          設定.LINE.トークン.はちまきbot,"リプライ",arg/* 関数.LINE.ID変換("グルラ_1-3") */
        )
       }else{
        ...
        line_HACHIMAKIbot: function(データ)
          {
            if(JSON.parse(データ.postData.contents).events) JSON.parse(データ.postData.contents).events.forEach(要素=>{
              if(要素.type=="message"&&要素.message.type=="text"){
                if(要素.message.text=="明日の時間割を教えて"){
                  時限処理.スケジュール(要素.replyToken)
                }
                関数.GSS.設定_最下行(
