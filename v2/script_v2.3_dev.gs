...
----------------<メモ>-------------------------
  今後機能が増えすぎた場合は時間ごとに処理をまとめるように変更することを想定
*/

//環境変数 //主要部は移植完了
  //ID
    const c_id_gss = "";//dev_curb
    const c_id_mainacct = "";
    const c_id_discacct = "";
...
//*共有関数
  ...
  //*trigger
    function f_sh_trigger_set(l_func,l_time){
      //ScriptApp.newTrigger(l_func).timeBased().at(l_time).create();//dev_curb
    };
...
//通常処理
  //*mail　移植完了
    ...
    function f_mail_message(l_mail){
      //送信アドレス別の処理分岐
      const c_from = LIB_Parser.data(l_mail.getFrom()).from("<").to(">").build();
      ...
      for(let i = 0; i < c_addresslist[0].length; i++){
        for(let k = 0; k < l_mail_to.length; k++){
          if(l_mail_to[k].slice(-1) == ">"){//アドレスの前に名前が入っていた場合の対処
            l_mail_to[k] = LIB_Parser.data(l_mail_to[k]).from("<").to(">").build();
          }
  ...
  //*rss　移植完了
    ...
    //取得元別処理
      function f_rss_nhk(l_rsstxt,l_lastget){
        const c_newsary = LIB_Parser.data(l_rsstxt).from("<item>").to("</item>").iterate();
        let l_returnary = [];
        for(let i = 0; i < c_newsary.length; i++){
          const c_new = {
            "title": LIB_Parser.data(c_newsary[i]).from("<title>").to("</title>").build(),
            "url": LIB_Parser.data(c_newsary[i]).from("<link>").to("</link>").build(),
            "body": LIB_Parser.data(c_newsary[i]).from("<description>").to("</description>").build(),
            "time": f_rss_dateconvert_A(LIB_Parser.data(c_newsary[i]).from("<pubDate>").to("</pubDate>").build()),
            "img": null
          };
      ...
      function f_rss_mynavi(l_rsstxt,l_lastget){
        let l_newsary = LIB_Parser.data(l_rsstxt).from("<item").to("</item>").iterate();
        let l_returnary = [];
        for(let i = 0; i < l_newsary.length; i++){
          l_newsary[i] = l_newsary[i].substr(l_newsary[i].indexOf(">")+1);
          const c_new = {
            "title": LIB_Parser.data(l_newsary[i]).from("<title>").to("</title>").build(),
            "url": LIB_Parser.data(l_newsary[i]).from("<link>").to("</link>").build(),
            "body": LIB_Parser.data(l_newsary[i]).from("<description>").to("</description>").build(),
            "time": f_rss_dateconvert_B(LIB_Parser.data(l_newsary[i]).from("<dc:date>").to("</dc:date>").build()),
            "img": LIB_Parser.data(l_newsary[i]).from("<img>").to("</img>").build()
          };
          if(l_lastget < c_new["time"]){
            l_returnary.push(c_new);
          }else{
  ...
  //*train　移植完了
    ...
    function f_pr_train(){
      ...
      let l_data =
        LIB_Parser.data(
          LIB_Parser.data(UrlFetchApp.fetch("https://traininfo.jreast.co.jp/delay_certificate/").getContentText("utf-8"))
  ...
  //*schedule
    ...
    function f_pr_schedule(){
      ...
      for(let i = 1; 0 <= i; i--){
        ...
        if(l_schedule[i][5] != "-"){
          l_school = "学校あり";
          l_field_ary[1] = f_sh_disc_field("行事予定",l_schedule[i][4]);
          ...
          for(let k = 0; k < 7; k++){
            ...
            l_field_ary[2+k] = f_sh_disc_field(
              String(k+1)+"時限目",
  ...
  //*sukasuka
    function f_bt_sukasuka(){
      f_sh_main("sukasuka");
    };

    function f_rs_sukasuka(){
      let l_ary = [];
      for(let i = 0; i < c_url_sukasuka.length; i++){
        l_ary[i] = UrlFetchApp.fetch(c_url_sukasuka[i][0]);
      };
      f_sh_cache_set("sukasuka",l_ary.join("<ary:str>"),5*60*60);
    };

    function f_pr_sukasuka(){
      if(f_sh_cache_get("sukasuka") == null){
        f_rs_sukasuka();
        return;
      };
      const c_cache = f_sh_cache_get("sukasuka").split("<ary:str>");
      let l_data = [];
      for(let i = 0; i < c_url_sukasuka.length; i++){
        l_data[i] = UrlFetchApp.fetch(c_url_sukasuka[i][0]);
        if(l_data[i] != c_cache[i]){
          f_sh_disc_send(
            "",
            f_sh_disc_message(
              "<@" + c_id_discacct + ">",
      ...
      f_sh_cache_set("sukasuka",l_data.join("<ary:str>"),6*60*60);
    };
  ...
  //*weather
    ...
    function f_pr_weather(){
      //メモ:LIB_Parser　iterate→配列　build→1つ
      const c_rss_dataary = LIB_Parser.data(UrlFetchApp.fetch("https://www.data.jma.go.jp/developer/xml/feed/regular.xml").getContentText("utf-8")).from("<entry>").to("</entry>").iterate();
      for(let i = 0; i < c_rss_dataary.length; i++){
        if(
            LIB_Parser.data(c_rss_dataary[i]).from("<updated>").to("</updated>").build() != f_sh_ps_get("weather_lastget")
          &&LIB_Parser.data(c_rss_dataary[i]).from("<title>").to("</title>").build() == "府県天気予報（Ｒ１）"
          &&LIB_Parser.data(c_rss_dataary[i]).from("<name>").to("</name>").build() == "横浜地方気象台"
        ){
          f_sh_ps_set("weather_lastget",LIB_Parser.data(c_rss_dataary[i]).from("<updated>").to("</updated>").build());
          const c_xml_dataary = LIB_Parser.data(UrlFetchApp.fetch(LIB_Parser.data(c_rss_dataary[i]).from("<id>").to("</id>").build()).getContentText("utf-8")).from("<TimeSeriesInfo>").to("</TimeSeriesInfo>").iterate();

          const c_area_3h_time = LIB_Parser.data(c_xml_dataary[3]).from("<DateTime>").to("</DateTime>").iterate();
          const c_area_3h_data_weather = LIB_Parser.data(c_xml_dataary[3]).from("<jmx_eb:Weather").to("</jmx_eb:Weather>").iterate();//[ refID="n" type="天気">]が残ってる & 後半は西部データ
          const c_area_3h_data_windspeed = LIB_Parser.data(c_xml_dataary[3]).from("<WindSpeedLevel").to("</WindSpeedLevel>").iterate();//[ description="毎秒１０から１４メートル" range="10 14" refID="1" type="風速階級">]が残ってる & 後半は西部データ

          const c_station_3h_time = LIB_Parser.data(c_xml_dataary[4]).from("<DateTime>").to("</DateTime>").iterate();
          const c_station_3h_data_temperate = LIB_Parser.data(c_xml_dataary[4]).from("<jmx_eb:Temperature").to("</jmx_eb:Temperature>").iterate();//[ description="２１度" refID="1" type="気温" unit="度">]が残ってる & 後半は小田原データ
